...
=========================================
Publish-ready research harness for SPY daily bars.

Key published configuration:
- train_years = 2
- atr_upper   = 1.0
- atr_lower   = 1.0
- tie_policy  = worst_case
- thresholds  = (0.55, 0.60, 0.65, 0.70, 0.75)

Latest canonical published run:
- run_tag = 20260315_124446
- LogReg DEV Sharpe = 0.51
- LightGBM DEV Sharpe = 0.16
...
class Config:
    ...
    end:         str   = "2025-01-02"   # avoids yfinance end-date off-by-one
    ...
    atr_upper:   float = 1.0
    atr_lower:   float = 1.0
    intraday_tie_policy: str = "worst_case"   # worst_case | best_case
...
def download_data(cfg: Config) -> pd.DataFrame:
    print("\n" + "=" * 60)
    print("STEP 1: DATA")
    print(f"  Downloading through {cfg.end} (avoids Dec-31 off-by-one)")
    print("=" * 60)
...
def engineer_features(df_raw: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    mu = f["ret_1d"].rolling(20).mean()
    sig = f["ret_1d"].rolling(20).std()
    ...
    f["adx"] = _adx(f, 14)
    f["regime"] = np.where(f["adx"] > 25, 1, np.where(f["vol_regime"] > 0.8, 2, 0))
    f = f.dropna().copy()
    ...
    if f[feat_cols].isna().any().any():
        raise AssertionError("NaN in features after dropna.")

    print(f"  Features: {len(feat_cols)} | Rows after warmup: {len(f)}")
...
def label_subset(df: pd.DataFrame, cfg: Config, subset_name: str) -> pd.DataFrame:
    ...
    if rate < 0.20:
        print("  ⚠ Too few LONG labels — label distribution is dangerously thin.")
    elif rate > 0.55:
        print("  ⚠ Too many LONG labels — class balance may be too optimistic.")
    else:
        print("  ✓ LONG rate acceptable")
    ...
    if n_classes < 2:
        raise ValueError(
            f"{subset_name}: labels contain only one class. "
            f"Check atr_upper / atr_lower / timeout_bars / slippage."
        )
...
def execution_engine(
    ...
    def _get(series: Optional[pd.Series], idx: Optional[int], default: Any = np.nan) -> Any:
    ...
    def _record(exit_i: int, exit_p: float, reason: str) -> None:
        ...
        trades.append({
            ...
            "regime_at_sig": int(df["regime"].iloc[signal_idx]) if "regime" in df.columns else -1,
        })
    ...
    for i in range(n):
        ...
        if flip_exit_for == i and in_pos:
            ...
            entry_price = ep
            entry_idx = i
            atr_sig = float(atrs[signal_idx]) if signal_idx is not None else float(atrs[max(i - 1, 0)])
            upper = ep + cfg.atr_upper * atr_sig
            lower = ep - cfg.atr_lower * atr_sig
            timeout_bar = (signal_idx + cfg.timeout_bars) if signal_idx is not None else (i + cfg.timeout_bars)
            mae = mfe = 0.0
            ...
            if exit_mode == "barrier":
                ...
                if hit_up and hit_dn:
                    ...
                    exit_p = _apply_exit_slippage(float(upper))
                    reason = "tp_entry_day"
                elif hit_dn:
                    exit_p = _apply_exit_slippage(float(lower))
                    reason = "sl_entry_day"
                elif timeout_bar is not None and i >= timeout_bar:
                    exit_p = _apply_exit_slippage(float(closes[i]))
                    reason = "timeout_entry_day"
                elif is_last:
                    exit_p = _apply_exit_slippage(float(closes[i]))
...
def bah_baseline(df: pd.DataFrame, cfg: Config) -> Tuple[pd.Series, List[Dict]]:
    ...
    trades = [{
        ...
        "exit_price": float(closes[n - 1]) * (1 - cfg.exit_slippage),
        "exit_reason": "end_of_period",
        "hold_days": n,
        "gross_ret": (float(closes[n - 1]) - entry_price) / entry_price,
        "net_ret": (float(closes[n - 1]) * (1 - cfg.exit_slippage) - entry_price) / entry_price - cfg.commission * 2,
        "mae": np.nan,
        "mfe": np.nan,
        "mae_r": np.nan,
        "mfe_r": np.nan,
...
def _bootstrap_ci(
    ...
    boots = [stat_fn(rng.choice(data, size=len(data), replace=True)) for _ in range(n_bootstrap)]
    lo_pct = (1 - ci) / 2 * 100
    hi_pct = (1 + ci) / 2 * 100
...
def metrics_engine(
    ...
    brier = None
    if (
        calib_probs is not None and calib_labels is not None
...
def run_baselines(df: pd.DataFrame, cfg: Config, label_prefix: str) -> Dict[str, Dict]:
    ...
    pnl, tr = execution_engine(df, mom_sig, cfg, exit_mode="barrier", strategy_name="10d_mom|barrier")
    ...
    pnl, tr = execution_engine(df, ma_sig, cfg, exit_mode="barrier", strategy_name="50/200_ma|barrier")
    results["50/200 MA | Barrier"] = metrics_engine(pnl, tr, "50/200 MA | Barrier", cfg)

    pnl, tr = execution_engine(df, mom_sig, cfg, exit_mode="flip", strategy_name="10d_mom|flip")
    ...
    pnl, tr = execution_engine(df, rand_sig, cfg, exit_mode="barrier", strategy_name="random|barrier")
    ...
    pnl, tr = execution_engine(df, null_sig, cfg, exit_mode="barrier", strategy_name="random_timing_matched")
    results["Random Timing (Matched Count)"] = metrics_engine(
        pnl, tr, "Random Timing (Matched Count)", cfg
    )

    print(f"\n  {'Strategy':<30} {'Sharpe':>7} {'Sortino':>8} {'MaxDD':>8} {'CAGR':>7} {'Trades':>7} {'Turn/yr':>8}")
    print("  " + "-" * 80)
    for m in results.values():
        print(
            f"  {m['label']:<30} {m['sharpe']:>7.2f} {m['sortino']:>8.2f} "
            f"{m['max_dd']:>7.1%} {m['cagr']:>6.1%} {m['n_trades']:>7} {m['turnover']:>7.1f}"
        )
...
def purged_walk_forward_probabilities(
    ...
    for fold_id, fold_start in enumerate(fold_starts):
        ...
        train_mask = ((dates >= fold_start - pd.DateOffset(years=cfg.train_years)) & (dates < fold_start))
        ...
        if len(np.unique(y_tr)) < 2:
            print(f"  [WARN] {model_type.upper()} fold {fold_id} skipped — training labels have one class.")
            continue
    ...
    if coverage < 0.50:
        print("  ⚠ Less than 50% of dev has OOS predictions — check train_years vs dev length")
...
def evaluate_threshold(
    ...
    daily_pnl, trades = execution_engine(
        ...
        cfg,
        exit_mode="barrier",
        force_exit_last_bar=True,
        prob_series=prob_eval,
        fold_series=fold_eval,
        label_series=label_eval,
        threshold=threshold,
...
def sweep_thresholds_from_prob_pack(
    ...
    if prob_pack.get("first_live") is None:
        ...
        print(f"  [WARN] {model_type_str} has no valid OOS predictions. Check train_years vs dev length. Skipping.")
        return cfg.thresholds[0], pd.DataFrame()

    model_type_str = prob_pack.get("model_type", "model").upper()
    window_str = eval_start.strftime("%Y-%m-%d") if eval_start is not None else "model-specific"

    print(f"\n  ── THRESHOLD SWEEP: {model_type_str} ──")
    print(f"  Evaluation window start: {window_str}")
    print(f"  Composite validity: trades>={cfg.min_trades}, PF>{cfg.min_pf}, exposure>{cfg.min_exposure:.0%}")
    print(f"\n  {'Thresh':>8} {'Sharpe':>8} {'Trades':>8} {'PF':>7} {'Expect':>9} {'Exposure':>9} {'Valid':>8}")
    print("  " + "-" * 64)
    ...
    for t in cfg.thresholds:
        ...
        print(
            f"  {t:>8.2f} {m['sharpe']:>8.2f} {m['n_trades']:>8} "
            f"{m['profit_factor']:>7.2f} {m['expectancy']:>9.4f} {m['exposure']:>8.1%} {flag:>8}"
        )
    ...
    if best_sharpe == -np.inf:
        print(f"  ⚠ No threshold passed composite validity — defaulting to {cfg.thresholds[0]}")
        best_thresh = cfg.thresholds[0]
...
def print_metrics(m: Dict[str, Any], baselines: Optional[Dict] = None) -> None:
    ...
    print(f"\n  ── {m['label'].upper()} ──")
    print("  Portfolio (daily equity curve):")
    print(
        f"    Sharpe:        {m['sharpe']:.2f}  "
        f"95%CI [{_safe_fmt(m['sharpe_ci'][0], '{:.2f}')} – {_safe_fmt(m['sharpe_ci'][1], '{:.2f}')}] "
        f"(iid bootstrap — heuristic only)"
    )
    ...
    ci_flag = "positive lower bound" if (not np.isnan(ci_lo) and ci_lo > 0) else "lower bound ≤ 0 — weak signal"
...
def trades_by_year_and_regime(
    ...
    if is_partial_first:
        print(f"  * {first_year} is a partial year (OOS window starts {first_date.date()})")
    has_bah = baseline_by_year is not None
    hdr = f"  {'StratRet':>9}  {'B&H Ret':>8}" if has_bah else f"  {'StratRet':>9}"
    print(f"  {'Year':>7} {'Trades':>8} {'WinRate':>9} {'PF':>7}{hdr}")
    ...
    for yr, g in td.groupby("year"):
        ...
        print(f"  {yr_label:>7} {len(g):>8} {(rets > 0).mean():>9.1%} {pf:>7.2f}{strat_col}{bah_col}")
    ...
    if "regime_at_sig" in td.columns:
        ...
        for reg, g in td.groupby("regime_at_sig"):
            ...
            print(f"  {rname:<12} {len(g):>8} {(rets > 0).mean():>9.1%} {rets.mean():>9.4f} {pf:>7.2f}")
...
def probability_bucket_tables(m: Dict[str, Any], threshold: float) -> str:
    ...
    lines.append(f"  {'Bucket':<12} {'Count':>7} {'HitRate':>9} {'AvgProb':>9} {'TradeCt':>8}")
    ...
    lines.append(f"  {'Bucket':<12} {'Count':>7} {'HitRate':>9} {'MeanRet':>9} {'PF':>7}")
    for (lo, hi), lbl in zip(buckets, blabels):
        ...
        wins = rets[rets > 0]
        loss = rets[rets < 0]
        hit = float(sub["label_truth"].mean()) if "label_truth" in sub.columns else float((rets > 0).mean())
        pf = wins.sum() / abs(loss.sum()) if loss.sum() != 0 else 999.0
        lines.append(f"  {lbl:<12} {count:>7} {hit:>9.1%} {rets.mean():>9.4f} {pf:>7.2f}")
...
def dev_vs_test_summary(
    ...
    print("  Test = 2024 final holdout (one shot, never touched before)")
    print("=" * 60)

    lr_test = (final_results or {}).get("logistic")
    lgbm_test = (final_results or {}).get("lgbm")

    print(f"\n  {'Metric':<24} {'LR Dev':>10} {'LR Test':>10} {'LGBM Dev':>10} {'LGBM Test':>10}")
    print("  " + "-" * 66)

    rows_def = [
        ("Sharpe",          "sharpe",        "{:>10.2f}"),
        ("Sharpe CI low",   None,            None),
    ...
    for name, key, fmt in rows_def:
        if key is None:
            ...
            def ci_lo(d):
                ...
                return _safe_fmt(ci[0], "{:>10.4f}" if "Exp" in name else "{:>10.2f}", "       N/A")

            print(f"  {name:<24}{ci_lo(lr_dev)}{ci_lo(lr_test)}{ci_lo(lgbm_dev)}{ci_lo(lgbm_test)}")
        else:
            print(f"  {name:<24}{fv(lr_dev, key, fmt)}{fv(lr_test, key, fmt)}{fv(lgbm_dev, key, fmt)}{fv(lgbm_test, key, fmt)}")
...
def run_final_test(
    ...
    best_thresh_lgbm: Optional[float],
    ...
    print(f"  LightGBM in test: {lgbm_in_test} ({'valid dev sweep' if lgbm_in_test else 'skipped — no valid dev sweep'})")
    print("█" * 60)
    ...
    for model_type, thresh in models_to_run:
        ...
        trades_by_year_and_regime(m, f"{model_type} TEST", baseline_by_year=bah_by_year_test)
...
def plot_results(
    ...
    fig.suptitle("Trading Research Engine — SPY (OOS-live window)", fontsize=12, fontweight="bold")
    ...
    for i, m in enumerate(dev_models):
        ax.plot(m["cum"].values, color=colors[i % len(colors)], lw=1.5, label=m["label"][:24])
    ...
    ax = axes[0, 1]
    for i, m in enumerate(dev_models):
        rs = m["daily_pnl"].rolling(90).apply(lambda x: x.mean() / (x.std() + 1e-12) * np.sqrt(252), raw=True)
        ax.plot(rs.values, color=colors[i % len(colors)], lw=1.2, label=m["label"][:24])
    ax.axhline(1.0, color="green", ls="--", alpha=0.5)
    ax.axhline(0.0, color="red", ls="--", alpha=0.4)
    ax.set_title("Rolling 90d Sharpe")
    ax.legend(fontsize=7)
    ax.grid(alpha=0.3)
    ...
    ax.set_title("MAE/MFE in Risk Units")
    ax.legend(fontsize=7)
    ax.grid(alpha=0.3)
...
def validate_config(cfg: Config) -> None:
    ...
    if any(not (0.0 < t < 1.0) for t in cfg.thresholds):
        raise ValueError(f"Config: all thresholds must be strictly in (0, 1). Got: {cfg.thresholds}")
    if tuple(sorted(cfg.thresholds)) != cfg.thresholds:
        raise ValueError(f"Config: thresholds must be sorted ascending. Got: {cfg.thresholds}")
    if len(set(cfg.thresholds)) != len(cfg.thresholds):
        raise ValueError(f"Config: thresholds must be unique. Got: {cfg.thresholds}")
    ...
    if cfg.atr_upper < 0.5 or cfg.atr_lower < 0.5:
        print("  [WARN] ATR barriers below 0.5 may produce unstable or degenerate labels.")
...
def main() -> None:
    ...
    print(f"  [CONFIG] entry_slippage: {cfg.entry_slippage:.3%} | exit_slippage: {cfg.exit_slippage:.3%}")
    ...
    lgbm_prob_pack = purged_walk_forward_probabilities(df_dev, feat_cols, cfg, "lgbm") if HAS_LGBM else None

    candidate_starts = [p.get("first_live") for p in [lr_prob_pack, lgbm_prob_pack] if p is not None]
    ...
    if lr_prob_pack is None or lr_prob_pack.get("first_live") is None:
        raise RuntimeError("LogReg produced no valid OOS predictions. Cannot continue — check train_years vs dev length.")
    ...
    lgbm_m = evaluate_threshold(df_dev, lgbm_pack, cfg, best_lgbm, eval_start=common_start) if lgbm_pack else None

    first_live_ts = common_start
    if first_live_ts is not None:
        df_dev_oos = df_dev.loc[first_live_ts:].copy()
        print(f"\n  Dev baselines restricted to OOS window ({first_live_ts.date()} onward)")
    else:
        df_dev_oos = df_dev
        print("  Could not determine OOS window — using full dev for baselines")
    ...
    print("=" * 60)
    print(f"  {'Strategy':<30} {'Sharpe':>7} {'Sortino':>8} {'MaxDD':>8} {'CAGR':>7} {'Turn/yr':>8}")
    print("  " + "-" * 74)
    ...
    if lgbm_m:
        ...
        print("\n  ── LogReg vs LightGBM ──")
        if sh_diff > 0.10:
            print(f"  LightGBM wins by {sh_diff:.2f} — use LightGBM.")
        elif sh_diff > 0:
            print(f"  Marginal gain ({sh_diff:.2f}) — prefer LogReg.")
        else:
            print("  LogReg wins/ties — complexity not earned.")
    ...
    if best_dev_sharpe >= 0.8:
        ...
        for m in baselines_test.values():
            print(f"  {m['label']:<30} {m['sharpe']:>7.2f} {m['max_dd']:>7.1%} {m['cagr']:>6.1%}")
    ...
    summary = {
        ...
        "dev_model_winner": ("lgbm" if lgbm_m and lgbm_m["sharpe"] > lr_m["sharpe"] + 0.10 else "logistic"),
        "final_model_winner_by_test_sharpe": (
            None if not final_results else
            max(
                ((mt, final_results[mt]["sharpe"]) for mt in final_results if final_results[mt].get("sharpe") is not None),
                key=lambda x: x[1],
                default=(None, None)
            )[0]
        ),
    ...
    print(f"  2.  DEV min signals/year:         {df_dev.groupby(df_dev.index.year)['label'].sum().min()}")
    print(f"  3.  B&H Sharpe (OOS window):      {_safe_fmt(bah_d['sharpe'] if bah_d else np.nan, '{:.2f}')}")
    print(f"  4.  Common OOS window start:      {common_start.date() if common_start else 'N/A'}")
    if lr_pack:
        print(f"  5a. LogReg raw OOS coverage:      {lr_pack['coverage']:.1%}  (model-specific, pre-alignment)")
    else:
        print("  5a. LogReg raw OOS coverage:      N/A (no valid OOS predictions)")
    if lgbm_pack:
        print(f"  5b. LightGBM raw OOS coverage:    {lgbm_pack['coverage']:.1%}  (model-specific, pre-alignment)")
    if common_start is not None:
        common_cov = float((df_dev.index >= common_start).mean())
        print(f"  5c. Common comparison coverage:   {common_cov:.1%}  (aligned window as share of full dev)")
    print(f"  6.  Best LogReg threshold:        {best_lr}")
    print(f"  7.  LogReg DEV Sharpe:            {lr_m['sharpe']:.2f}")
    print(f"  7b. LogReg Sharpe 95% CI:         [{_safe_fmt(lr_m['sharpe_ci'][0], '{:.2f}')} – {_safe_fmt(lr_m['sharpe_ci'][1], '{:.2f}')}]")
